...
export const sketch = (p: p5) => {
  ...
  p.draw = () => {
    ...
    p.background(0)
    p.stroke(255,100)
    p.ambientLight(255,1)
    p.pointLight(50,50,50,width/2,height/2,1000)
    p.specularMaterial(100,100,255,10)
    p.rotate(t%360, p.createVector(width/2, height/2, 1000))
    p.ellipsoid(200*Math.cos(t)+500, 200*Math.cos(t)+500, 200*Math.cos(t)+500, width/2, height/2 )
    p.rotate(t+180%360, p.createVector(width/2, height/2, 1000))
    p.ellipsoid(Math.cos(t)+500, Math.cos(t)+500, Math.cos(t)+500, width/2, height/2 )
    p.specularMaterial(255, 100, 255, 200)
    p.box(500*Math.sin(t)+20, 500*Math.sin(t)+20, 500*Math.sin(t)+20, width/2, height/2 )
    t+=0.1 // counter
  }
