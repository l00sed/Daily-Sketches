...
export const sketch = (p: p5) => {
  ...
  p.draw = () => {
    ...
    for (let u_segs=0; u_segs < width; u_segs+=width/20+Math.sin(t)) {
      for (let v_segs=0; v_segs < height; v_segs+=height/20+Math.sin(t)) {
        p.fill(u_segs%255,u_segs%200,u_segs%120)
        p.stroke(u_segs%255, u_segs%200, u_segs%120)
        p.ellipse(u_segs,v_segs,t%20)
        p.rotate(Math.sin(t))
      }
    }
