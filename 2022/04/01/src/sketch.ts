...
export const sketch = (p: p5) => {
  ...
  let xSlide:number = 0
  let zSlide:number = 200

  p.draw = () => {
    // Define render logic for your sketch here
    p.rotate(t*0.1)
    p.lightFalloff(t%1.0, 0.0, 0.0)
    p.pointLight(t*10%200,t*100%200,t*1000%200,0,0,zSlide)
    p.ambientLight(40,40,40,200)

    p.translate(xSlide,0,0)
    p.specularMaterial(100,100,100,255)
    p.noStroke()
    p.sphere(10, 20, 20)

    t+=1 // counter
    xSlide = (900*Math.cos(t))
    zSlide = (-100*Math.cos(t)+100*Math.cos(t))
  }
